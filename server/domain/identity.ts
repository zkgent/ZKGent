...
export function upsertWalletUser(opts: {
  ...
  const existing = db
    .prepare(`SELECT * FROM wallet_users WHERE wallet_address = ?`)
...
export function getAllWalletUsers(limit = 50): WalletUser[] {
  return db
    .prepare(`SELECT * FROM wallet_users ORDER BY last_seen_at DESC LIMIT ?`)
