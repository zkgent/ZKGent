...
treasuryRouter.get("/:id", (req, res) => {
  try {
    const row = db.prepare("SELECT * FROM treasury_routes WHERE id = ?").get(req.params.id) as Record<string, unknown> | undefined;
...
treasuryRouter.post("/", (req, res) => {
  try {
    ...
    if (!body.source?.toString().trim()) return res.status(400).json({ error: "source is required" });
    if (!body.destination?.toString().trim()) return res.status(400).json({ error: "destination is required" });
    ...
    const row = db.prepare("SELECT * FROM treasury_routes WHERE id = ?").get(id) as Record<string, unknown>;
