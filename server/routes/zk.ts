...
import {
  createSettlementMemoText,
  ensureOperatorFunded,
  getOperatorAddress,
  getLatestTxs,
} from "../domain/solana_tx.js";
...
zkRouter.get("/proofs/:id/verify", async (req, res) => {
  try {
    ...
    try {
      ...
      return res.status(400).json({ error: "proof_data_not_json" });
    }
...
zkRouter.post("/settlement/initiate", requireApprovedWallet, async (req, res) => {
  ...
  try {
    ...
    if (initiated_by_wallet) {
      db.prepare(`UPDATE zk_settlements SET initiated_by_wallet = ? WHERE id = ?`).run(
        initiated_by_wallet,
        record.id,
      );
    }
...
zkRouter.post("/tx/prepare", requireApprovedWallet, async (req, res) => {
  ...
  try {
    ...
    const tx = await buildSettlementMemoTx(memo, wallet_address);
    // Serialize without requiring all signatures, the authenticated wallet will
    // sign and pay fees client-side.
    ...
    db.prepare(
      `UPDATE zk_settlements SET status = 'signing_requested', signing_request_id = ?, initiated_by_wallet = COALESCE(initiated_by_wallet, ?) WHERE id = ?`,
 ...
 *
 * Body: { request_id, tx_signature, network }
 */
zkRouter.post("/tx/confirm", requireApprovedWallet, async (req, res) => {
  ...
  const signing_request = db
    ...
    .get(request_id) as SigningRequestRow | undefined;
  if (!signing_request) return res.status(404).json({ error: "signing_request_not_found" });

  const confirmError = getSigningRequestConfirmError(signing_request, wallet_address);
  if (confirmError) {
    const statusByError: Record<NonNullable<typeof confirmError>, number> = {
      signing_request_not_pending: 409,
      signing_request_already_responded: 409,
      signing_request_expired: 410,
      signing_request_wallet_missing: 500,
      wallet_session_mismatch: 403,
    };
    return res.status(statusByError[confirmError]).json({ error: confirmError });
  }

  const { getTxExplorerUrl } = await import("../domain/solana.js");
  const net = (network ?? getSolanaConfig().network) as ReturnType<
    typeof getSolanaConfig
  >["network"];
  ...
  if (signing_request.settlement_id) {
    ...
    const settlement = db
      .prepare(`SELECT commitment, nullifier, proof_id FROM zk_settlements WHERE id = ? LIMIT 1`)
      .get(signing_request.settlement_id) as
      | { commitment: string | null; nullifier: string | null; proof_id: string | null }
      | undefined;
    recordOnChainTx({
      ...
      memoData: settlement
        ? createSettlementMemoText({
            settlement_id: signing_request.settlement_id,
            commitment_short: (settlement.commitment ?? "pending").slice(0, 12),
            nullifier_short: (settlement.nullifier ?? "pending").slice(0, 12),
            proof_id: settlement.proof_id ?? "pending",
            version: "v1",
          })
        : signing_request.settlement_id,
      explorerUrl,
    });
  }
  ...
  logActivity({
    category: "settlement",
    event: "wallet_signed_and_submitted",
