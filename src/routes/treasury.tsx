...
function AddRouteModal({
  ...
  onClose: () => void;
  onCreated: (r: TreasuryRoute) => void;
}) {
  ...
  const submit = async () => {
    ...
    setErr(null);
    try {
      const r = await api.treasury.create({ ...form, walletAddress } as Parameters<
        typeof api.treasury.create
      >[0]);
      onCreated(r);
    } catch (e: unknown) {
        ...
        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6">
          ...
          <motion.div variants={item}>
            ...
            <div className="rounded-2xl border border-hairline overflow-hidden">
              {loading ? (
                [0, 1, 2].map((i) => (
                  <div key={i} className="p-5 border-b border-hairline animate-pulse">
                    <div className="h-4 w-1/2 rounded bg-surface-elevated mb-2" />
                ...
                <div className="flex flex-col items-center justify-center py-14 gap-2">
                  <p className="text-[13px] text-muted-foreground">
                    Connect a wallet to view treasury routes
                  </p>
                  <p className="text-[11px] text-muted-foreground/50">
                    Routes are scoped per wallet — each wallet sees only its own routes
                  </p>
                </div>
              ) : routes.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-14 gap-2">
                    ...
                    return (
                      <div key={r.id} className="p-5 hover:bg-surface-elevated transition-colors">
                        ...
                        <div className="flex items-center gap-2 text-[11px] text-muted-foreground flex-wrap">
                          {r.source && (
                            <span className="rounded bg-surface-elevated border border-hairline px-2 py-0.5 font-mono text-[10px]">
                              {r.source}
                            </span>
                          )}
                        ...
                        {r.lastMovedAt && (
                          <p className="mt-2 font-mono text-[10px] text-muted-foreground/40">
                            Last movement: {fmtDate(r.lastMovedAt)}
                          </p>
                        )}
                      </div>
                    );
