...
const STATUS_META: Record<string, { label: string; dot: string; text: string; badge: string }> = {
  ...
  not_connected: {
    ...
    badge: "border-hairline bg-surface text-muted-foreground",
  },
};
...
function AddCounterpartyModal({
  ...
  const submit = async () => {
    ...
    setErr(null);
    try {
      const c = await api.counterparties.create({ ...form, createdByWallet } as Parameters<
        typeof api.counterparties.create
      >[0]);
      onCreated(c);
    } catch (e: unknown) {
      ...
      >
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            >
              {["Institutional", "Corporate", "Operator", "Remittance"].map((o) => (
          ...
          <div>
            ...
            <select
              ...
              className="w-full rounded-lg border border-hairline bg-background px-3 py-2.5 text-[13px] text-foreground outline-none focus:border-emerald/50 appearance-none"
            >
              {["Vendor", "Partner", "Payee", "Recipient", "Service"].map((o) => (
                <option key={o}>{o}</option>
              ))}
            </select>
          </div>
            ...
            <div className="divide-y divide-hairline">
              {loading ? (
                [0, 1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-4 px-5 py-4 animate-pulse"
                  >
                ...
                <div className="flex flex-col items-center justify-center py-12 gap-2">
                  <p className="text-[13px] text-muted-foreground">
                    Connect a wallet to view counterparties
                  </p>
                  <p className="text-[11px] text-muted-foreground/50">
                    Counterparties are scoped per wallet — each wallet sees only its own list
                  </p>
                </div>
              ) : visible.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 gap-2">
                  <p className="text-[13px] text-muted-foreground">
                    {counterparties.length === 0 ? "No counterparties yet" : "No matches"}
                  </p>
                  {counterparties.length === 0 && (
                    <p className="text-[11px] text-muted-foreground/50">
                      Add your first counterparty above
                    </p>
                  )}
                </div>
              ) : (
                visible.map((c) => {
                  ...
                  return (
                    <div
                      key={c.id}
                      className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-4 px-5 py-4 items-center hover:bg-surface-elevated transition-colors cursor-default"
                    >
                      <div>
                        <p className="text-[13px] font-medium text-foreground">{c.name}</p>
                        <p className="font-mono text-[10px] text-muted-foreground/50">
                          {c.id} · Added {fmtDate(c.createdAt)}
                        </p>
                      </div>
                      <span className="text-[11px] text-muted-foreground">{c.type}</span>
                      <span className="text-[11px] text-muted-foreground">{c.region || "—"}</span>
                      <span className="rounded-full border border-hairline bg-surface-elevated px-2.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
                        {c.relationship}
                      </span>
                      <div
                        className={cn(
