...
const TRUST_META: Record<CeremonyState["trust_level"], { label: string; tone: string; ring: string; bg: string }> = {
  none: { label: "No ceremony", tone: "text-muted-foreground", ring: "border-hairline", bg: "bg-surface/40" },
  single_party: { label: "Single-party only", tone: "text-yellow-300", ring: "border-yellow-500/30", bg: "bg-yellow-500/[0.06]" },
  single_party_plus_beacon: { label: "1 contributor + beacon", tone: "text-cyan", ring: "border-cyan/30", bg: "bg-cyan/[0.06]" },
  multi_party: { label: "Multi-party (no beacon yet)", tone: "text-cyan", ring: "border-cyan/30", bg: "bg-cyan/[0.06]" },
  multi_party_plus_beacon: { label: "Multi-party + beacon", tone: "text-emerald", ring: "border-emerald/30", bg: "bg-emerald/[0.06]" },
};
...
function CeremonyPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/zk/ceremony")
      ...
      .then((d: CeremonyState) => { if (!cancelled) setState(d); })
    ...
    <main className="relative min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto max-w-3xl px-6 py-16">
        ...
        <p className="mt-3 max-w-2xl text-[14px] leading-relaxed text-muted-foreground">
          The ZKGent transfer circuit needs a Phase-2 trusted setup. As long as ANY ONE
          contributor (or the public beacon) was honest about destroying their secret randomness,
          the circuit is sound and proofs cannot be forged. This page shows the current chain
          end-to-end. Anyone can independently verify it with{" "}
          <code className="rounded bg-background px-1.5 py-0.5 font-mono text-[11px] text-foreground">npm run ceremony:verify</code>.
        </p>
          ...
          <>
            {!state.hashes_consistent && (
              <div className="mt-8 rounded-2xl border border-red-500/40 bg-red-500/[0.08] p-5">
                <p className="font-mono text-[10px] uppercase tracking-widest text-red-300 mb-2">⚠ Integrity check failed</p>
                <p className="text-[13px] leading-relaxed text-red-200/90">
                  The on-disk <code className="font-mono text-[11px]">transfer_final.zkey</code> and/or{" "}
                  <code className="font-mono text-[11px]">verification_key.json</code> do NOT match the hashes recorded
                  in the manifest. The proving artifact in production right now is NOT what this chain attests to.
                  Re-run <code className="font-mono text-[11px]">npm run ceremony:verify</code> to investigate.
                </p>
              </div>
            )}
            ...
            <p className="mt-12 font-mono text-[10px] uppercase tracking-widest text-muted-foreground/60">
              Manifest: {state.manifest_path_rel} · Fetched {new Date(state.fetched_at).toLocaleString()}
            </p>
          </>
        )}
      </div>
  ...
  return (
    <div className={`mt-8 rounded-2xl border ${meta.ring} ${meta.bg} p-6`}>
      <div className="flex items-center gap-3">
        <span className={`h-2 w-2 rounded-full ${meta.tone === "text-emerald" ? "bg-emerald" : meta.tone === "text-cyan" ? "bg-cyan" : meta.tone === "text-yellow-300" ? "bg-yellow-400 animate-pulse" : "bg-muted-foreground"}`} />
...
function ChainOverview({ state, onCopy, copied }: { state: CeremonyState; onCopy: (s: string, l: string) => void; copied: string | null }) {
  const setupHash = state.contributions[0]?.prev_zkey_hash ?? null;
  return (
    <section className="mt-10">
      <h2 className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-4">Chain</h2>
...
function ChainStep({
  index, label, desc, hash, arrow, highlight, onCopy, copied,
}: {
  index: string; label: string; desc: string;
  hash: string | null; arrow?: string; highlight?: boolean;
  onCopy: (s: string, l: string) => void; copied: string | null;
}) {
  return (
    <div className={`flex items-center gap-3 rounded-xl border ${highlight ? "border-cyan/30 bg-cyan/[0.04]" : "border-hairline bg-surface/40"} p-3`}>
      <span className={`min-w-16 font-mono text-[10px] uppercase tracking-widest ${highlight ? "text-cyan" : "text-muted-foreground"}`}>
        {index}
      </span>
...
function ContributorList({ state, onCopy, copied }: { state: CeremonyState; onCopy: (s: string, l: string) => void; copied: string | null }) {
        ...
        {state.contributions.map((c) => (
          <article key={c.index} className="rounded-2xl border border-hairline bg-surface p-5">
            <header className="flex flex-wrap items-center gap-3">
              <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">#{String(c.index).padStart(4, "0")}</span>
              ...
              <span className="ml-auto font-mono text-[10px] text-muted-foreground">{new Date(c.contributed_at).toLocaleString()}</span>
            </header>
            {c.note && (
...
function BeaconDetails({ beacon, onCopy, copied }: { beacon: Beacon; onCopy: (s: string, l: string) => void; copied: string | null }) {
  return (
    <section className="mt-10">
      <h2 className="font-mono text-[10px] uppercase tracking-widest text-cyan mb-4">Final beacon</h2>
      <article className="rounded-2xl border border-cyan/30 bg-cyan/[0.04] p-5">
        <header className="flex flex-wrap items-center gap-3">
          <span className="text-[14px] font-semibold text-foreground">Solana mainnet finalized blockhash</span>
          <span className="ml-auto font-mono text-[10px] text-muted-foreground">{new Date(beacon.applied_at).toLocaleString()}</span>
        </header>
        <p className="mt-2 text-[12.5px] leading-relaxed text-muted-foreground">
          A blockhash chosen AFTER all contributions were collected. Public, unpredictable in advance, and verifiable by anyone hitting Solana RPC. The beacon prevents any contributor from controlling the final randomness.
        </p>
        ...
        <p className="mt-4 rounded-lg bg-background/60 p-3 font-mono text-[11px] leading-relaxed text-muted-foreground">
          beacon = SHA256(<span className="text-foreground">"zkgent-ceremony-beacon-v1"</span> ‖ <span className="text-foreground">{beacon.slot}</span> ‖ <span className="text-foreground">"{beacon.blockhash}"</span>)
        </p>
...
function FinalArtifact({ state, onCopy, copied }: { state: CeremonyState; onCopy: (s: string, l: string) => void; copied: string | null }) {
  return (
    <section className="mt-10">
      <h2 className="font-mono text-[10px] uppercase tracking-widest text-emerald mb-4">Production artifact</h2>
      <article className="rounded-2xl border border-emerald/30 bg-emerald/[0.04] p-5">
        ...
        <dl className="mt-4 grid grid-cols-1 gap-2">
          <KV
            ...
            onClick={() => state.derived_final_zkey_hash && onCopy(state.derived_final_zkey_hash, "final-zkey")}
    ...
    <section className="mt-10">
      ...
      <div className="rounded-2xl border border-hairline bg-surface/60 p-6 space-y-4">
        <p className="text-[13px] leading-relaxed text-muted-foreground">
          Anyone can add a contribution. Each contribution requires the previous zkey, runs locally on your machine, and produces a new zkey. The OS-random entropy you generate during the contribution exists only in your process memory and is destroyed when the script exits — that is the "destroy your toxic waste" step.
        </p>
        ...
        <ol className="space-y-3 text-[13px] text-muted-foreground">
          <li className="flex gap-3">
            ...
            <div>Clone the repo & install: <code className="rounded bg-background px-1.5 py-0.5 font-mono text-[11px] text-foreground">git clone … && npm install</code></div>
          </li>
          <li className="flex gap-3">
...
{`npm run ceremony:contribute -- \\
  ...
  --note "Optional context"`}
          ...
          <li className="flex gap-3">
            ...
            <div>
              Verify locally: <code className="rounded bg-background px-1.5 py-0.5 font-mono text-[11px] text-foreground">npm run ceremony:verify</code>
            </div>
          </li>
          <li className="flex gap-3">
            <span className="font-mono text-[11px] text-emerald shrink-0 w-5">4.</span>
            <div>
              Open a pull request adding your <code className="rounded bg-background px-1.5 py-0.5 font-mono text-[11px] text-foreground">contribution_NNNN.zkey</code> + attestation JSON. The maintainers re-verify and merge.
            </div>
          </li>
          <li className="flex gap-3">
            <span className="font-mono text-[11px] text-emerald shrink-0 w-5">5.</span>
            <div>
              Once enough contributions are collected, the maintainers run <code className="rounded bg-background px-1.5 py-0.5 font-mono text-[11px] text-foreground">npm run ceremony:beacon</code> to apply a fresh Solana mainnet beacon and finalize.
            </div>
          </li>
        </ol>

        <div className="rounded-lg border border-yellow-500/20 bg-yellow-500/[0.04] p-3 text-[12px] leading-relaxed text-yellow-200/80">
          <strong className="text-yellow-300">Soundness note:</strong> the security of the final zkey holds as long as
          ANY ONE contributor (or the beacon) was honest about destroying their secret. More contributors = stronger.
          One honest party is enough; you don't need to trust everyone.
        </div>
      </div>
    </section>
  );
...
function KV({ label, value, onClick, copied, full, highlight }: {
  label: string; value: string;
  onClick?: () => void; copied?: boolean; full?: boolean;
  highlight?: "ok" | "bad";
}) {
  const tone = highlight === "ok" ? "text-emerald" : highlight === "bad" ? "text-red-300" : "text-foreground/85";
  return (
    <div className={`flex items-center justify-between gap-3 rounded-lg border border-hairline/60 bg-background/40 px-3 py-2 ${full ? "sm:col-span-2" : ""}`}>
