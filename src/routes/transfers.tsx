  ...
  return (
    <div className="min-h-full bg-background">
      <div className="max-w-7xl mx-auto px-5 py-8 lg:px-8">
        ...
        {error && <div className="mb-4 rounded-xl border border-red-500/20 bg-red-500/5 px-4 py-3 text-[13px] text-red-400">{error}</div>}
        ...
        <div className="flex gap-4">
          <div className={`flex-1 min-w-0 rounded-2xl border border-hairline overflow-hidden ${selected ? "hidden lg:block" : ""}`}>
            ...
            {loading ? (
              [0, 1, 2, 3].map((i) => (
                <div key={i} className="grid grid-cols-[1fr_auto_auto_auto] gap-4 px-4 py-3.5 border-b border-hairline animate-pulse">
              ...
              <div className="flex flex-col items-center justify-center py-16 gap-2">
                ...
                {filter === "all" && <p className="text-[11px] text-muted-foreground/50">Create your first transfer using the button above</p>}
              </div>
            ) : (
                ...
                {visible.map((t) => {
                  ...
                  return (
                    <button key={t.id} onClick={() => setSelected(selected === t.id ? null : t.id)}
                      ...
                      <div>
                        <p className="font-mono text-[12px] font-medium text-foreground">{t.reference}</p>
                      ...
                      <span className="self-center font-mono text-[11px] text-muted-foreground">{t.asset}</span>
                      <span className="self-center font-mono text-[10px] text-muted-foreground/60">{fmtDate(t.createdAt)}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }}
              ...
              <div className="flex items-center justify-between border-b border-hairline px-4 py-3">
                <div>
                  ...
                  <p className="text-[13px] font-medium text-foreground">{selectedTransfer.reference}</p>
                </div>
              ...
              <div className="p-4 space-y-0">
                {[
                  { l: "Status", v: STATUS_META[selectedTransfer.status]?.label ?? selectedTransfer.status, accent: STATUS_META[selectedTransfer.status]?.text },
                  ...
                  { l: "Settled", v: selectedTransfer.settledAt ? fmtDate(selectedTransfer.settledAt) : "—" },
                  { l: "Notes", v: selectedTransfer.notes || "—" },
                ].map(({ l, v, accent }) => (
                  <div key={l} className="flex items-start justify-between gap-4 py-2 border-b border-hairline last:border-0">
