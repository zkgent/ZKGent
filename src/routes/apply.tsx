...
const USE_CASES = [
  {
    ...
    desc: "Confidential salary and contractor disbursements",
  },
  { id: "treasury", label: "Treasury Operations", desc: "Private multi-sig treasury movements" },
  {
    id: "merchant",
    label: "Merchant Payments",
    desc: "Shielded point-of-sale and e-commerce settlement",
  },
  { id: "remittance", label: "Remittance", desc: "Cross-border transfers without public exposure" },
  {
    id: "transfers",
    label: "High-Sensitivity Transfers",
    desc: "Institutional and high-value confidential flows",
  },
];
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected
          ? "border-emerald/50 bg-emerald/[0.08]"
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <TextInput
          ...
          placeholder="e.g. CFO, Head of Engineering, Founder"
        />
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <SelectInput
          value={data.currentRail}
          onChange={(v) => update({ currentRail: v })}
          options={RAILS}
          placeholder="Select"
        />
      </div>
      <div>
        ...
        <SelectInput
          value={data.privacyConcern}
          onChange={(v) => update({ privacyConcern: v })}
          options={CONCERNS}
          placeholder="Select"
        />
      </div>
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start gap-3 rounded-xl border border-emerald/20 bg-emerald/5 p-4">
        ...
        <p className="text-[12px] leading-relaxed text-muted-foreground">
          Your application is reviewed by the ZKGENT core team. All information is handled with
          strict confidentiality. We typically respond within 5–7 business days.
        </p>
      </div>
    </div>
  );
    ...
    <div className="mt-4 flex items-start gap-3 rounded-xl border border-hairline bg-surface p-4">
      ...
      <div className="flex-1">
        <p className="text-[12px] font-medium text-foreground">
          {wallet
            ? "Wallet will be linked to this application"
            : "Connect a wallet to skip the linking step later (optional)"}
        </p>
        {wallet ? (
...
function canAdvance(step: number, data: ApplicationFormData): boolean {
  ...
  if (step === 2)
    return !!(
      ...
      data.currentRail &&
      data.privacyConcern &&
      data.whyConfidential?.trim().length >= 10
    );
  return true;
}
    ...
    return (
      <div className="flex min-h-full flex-col items-center justify-center px-6 py-20 text-center gap-4">
        ...
        <div className="flex gap-3 mt-2">
          ...
          <button
            onClick={() => {
              /* let them reapply with new email */ setSubmitError(null);
            }}
