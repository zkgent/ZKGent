        ...
        <div key={label} className="flex items-center">
          <div className="flex flex-col items-center gap-1.5">
            <div className={cn(
              ...
              i < current ? "border-emerald bg-emerald/20 text-emerald"
                : i === current ? "border-emerald bg-emerald text-background"
            ...
            <span className={cn(
              "hidden sm:block font-mono text-[9px] uppercase tracking-wider",
              i === current ? "text-emerald" : i < current ? "text-emerald/60" : "text-muted-foreground/40"
            )}>{label}</span>
          </div>
          {i < STEPS.length - 1 && (
            <div className={cn("mx-2 mb-4 h-px w-8 sm:w-14 transition-colors duration-500",
...
function FieldLabel({ children, required }: { children: React.ReactNode; required?: boolean }) {
  return (
    <label className="mb-1.5 block font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
      {children}{required && <span className="ml-1 text-emerald">*</span>}
    </label>
  );
}
...
function RadioCard({ selected, onSelect, label, desc }: {
  ...
  return (
    <button type="button" onClick={onSelect}
      className={cn("w-full text-left rounded-xl border p-4 transition-all duration-200",
        selected ? "border-emerald/50 bg-emerald/[0.08]" : "border-hairline bg-surface hover:border-emerald/25 hover:bg-surface-elevated")}>
      <div className="flex items-start gap-3">
        <div className={cn("mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full border transition-all",
...
function Step1({ data, update }: { data: any; update: any }) {
  return (
    <div className="space-y-5">
      <div><FieldLabel required>Full Name</FieldLabel>
        <TextInput value={data.fullName} onChange={(v) => update({ fullName: v })} placeholder="Your full name" /></div>
      <div><FieldLabel required>Work Email</FieldLabel>
        <TextInput type="email" value={data.workEmail} onChange={(v) => update({ workEmail: v })} placeholder="you@company.com" /></div>
      <div><FieldLabel required>Company / Project</FieldLabel>
        <TextInput value={data.company} onChange={(v) => update({ company: v })} placeholder="Your company or project name" /></div>
      <div><FieldLabel required>Role</FieldLabel>
        <TextInput value={data.role} onChange={(v) => update({ role: v })} placeholder="e.g. CFO, Head of Engineering, Founder" /></div>
    </div>
  );
}
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-2 space-y-2">
          {USE_CASES.map((uc) => (
            <RadioCard key={uc.id} selected={data.useCase === uc.id}
              onSelect={() => update({ useCase: uc.id })} label={uc.label} desc={uc.desc} />
          ))}
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div><FieldLabel required>Team Size</FieldLabel>
          <SelectInput value={data.teamSize} onChange={(v) => update({ teamSize: v })} options={TEAM_SIZES} placeholder="Select" /></div>
        <div><FieldLabel required>Region</FieldLabel>
          <SelectInput value={data.region} onChange={(v) => update({ region: v })} options={REGIONS} placeholder="Select" /></div>
      </div>
    </div>
  );
}
...
function Step3({ data, update }: { data: any; update: any }) {
  return (
    <div className="space-y-5">
      <div><FieldLabel required>Monthly Payment Volume</FieldLabel>
        <SelectInput value={data.monthlyVolume} onChange={(v) => update({ monthlyVolume: v })} options={VOLUMES} placeholder="Select range" /></div>
      <div><FieldLabel required>Current Payment Rail</FieldLabel>
        <SelectInput value={data.currentRail} onChange={(v) => update({ currentRail: v })} options={RAILS} placeholder="Select" /></div>
      <div><FieldLabel required>Biggest Privacy Concern</FieldLabel>
        <SelectInput value={data.privacyConcern} onChange={(v) => update({ privacyConcern: v })} options={CONCERNS} placeholder="Select" /></div>
      <div><FieldLabel required>Why confidential payments matter to you</FieldLabel>
        <textarea value={data.whyConfidential} onChange={(e) => update({ whyConfidential: e.target.value })}
          placeholder="Describe your specific situation or use case in a few sentences..."
          rows={4}
          className="w-full rounded-lg border border-hairline bg-surface px-4 py-3 text-[14px] text-foreground placeholder:text-muted-foreground/40 outline-none transition-all focus:border-emerald/50 focus:ring-1 focus:ring-emerald/20 resize-none" /></div>
    </div>
  );
}
...
function canAdvance(step: number, data: any): boolean {
  ...
  if (step === 2) return !!(data.monthlyVolume && data.currentRail && data.privacyConcern && data.whyConfidential?.trim().length >= 10);
  return true;
}
...
function ApplyPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 409) {
          setApplicationId(json.existingId);
          navigate({ to: "/submitted" });
          return;
        }
        const msg = json.details ? json.details.join(". ") : json.error;
        setSubmitError(msg);
        return;
      }
  ...
  if (applicationId) {
    return (
      <div className="flex min-h-full flex-col items-center justify-center px-6 py-20 text-center gap-4">
        <div className="flex h-12 w-12 items-center justify-center rounded-full border border-emerald/30 bg-emerald/10 mx-auto">
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
            <path d="M4 10l4 4 8-8" stroke="oklch(0.78 0.16 160)" strokeWidth="2" strokeLinecap="round" />
          </svg>
        </div>
        <p className="text-[14px] text-foreground font-medium">You already have an application on file.</p>
        <p className="font-mono text-[11px] text-muted-foreground">ID: {applicationId}</p>
        <div className="flex gap-3 mt-2">
          ...
          <button onClick={() => { /* let them reapply with new email */ setSubmitError(null); }}
            className="inline-flex items-center gap-2 rounded-full border border-hairline px-5 py-2.5 text-[13px] text-muted-foreground hover:text-foreground transition">
            Use different email
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-full bg-background">
      ...
      <div className="mx-auto max-w-xl px-5 py-10">
        <div className="mb-8">
          <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground/60 mb-1">Early Access Application</p>
          <h1 className="font-display text-2xl font-semibold text-foreground">{STEPS[step]}</h1>
        </div>
        ...
        <div className="relative overflow-hidden">
          <AnimatePresence mode="wait" custom={direction}>
            <motion.div key={step} custom={direction} variants={variants}
              ...
              {step === 0 && <Step1 data={formData} update={updateForm} />}
              {step === 1 && <Step2 data={formData} update={updateForm} />}
              {step === 2 && <Step3 data={formData} update={updateForm} />}
        ...
        <div className="mt-8 flex items-center justify-between gap-4">
          <button onClick={goPrev}
            className={cn("flex items-center gap-2 rounded-full border border-hairline px-5 py-2.5 text-[13px] font-medium text-muted-foreground transition-all hover:text-foreground hover:border-foreground/20",
              step === 0 && "invisible")}>
          ...
          {step < STEPS.length - 1 ? (
            <button onClick={goNext} disabled={!canAdvance(step, formData)}
            ...
            <button onClick={handleSubmit} disabled={submitting}
              className="group relative inline-flex items-center gap-2 overflow-hidden rounded-full bg-foreground px-6 py-2.5 text-[13px] font-medium text-background transition-all hover:shadow-[0_0_30px_-8px_rgba(255,255,255,0.4)] disabled:opacity-60">
              <span className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-emerald/30 to-transparent transition-transform duration-700 group-hover:translate-x-full" />
              <span className="relative">{submitting ? "Submitting…" : "Request Early Access"}</span>
              {!submitting && <span className="relative transition-transform group-hover:translate-x-0.5">→</span>}
            </button>
          )}
        </div>
      </div>
