...
const STATUS_META: Record<string, { label: string; dot: string; badge: string }> = {
  ...
  pending: {
    ...
    badge: "border-yellow-500/20 bg-yellow-500/8 text-yellow-400",
  },
  settled: {
    label: "Settled",
    dot: "bg-emerald/60",
        ...
        <div className="space-y-3">
          {[
            ...
            { k: "scheduledDate", l: "Scheduled date", p: "Disbursement date", type: "date" },
            {
              k: "recipientCount",
              l: "Recipient count",
              p: "Number of recipients",
              type: "number",
            },
            {
              k: "approvalThreshold",
              l: "Approval threshold",
              p: "Required signers",
              type: "number",
            },
          ].map(({ k, l, p, type }) => (
        ...
        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6">
          ...
          <motion.div variants={item} className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {[
              ...
              {
                label: "Next Run",
                value: nextDraft?.scheduledDate ? fmtDate(nextDraft.scheduledDate) : "—",
                sub: nextDraft?.name ?? "No pending batches",
              },
            ].map((s) => (
                ...
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-14 gap-2">
                    <p className="text-[13px] text-muted-foreground">
                      Connect a wallet to view payroll batches
                    </p>
                    <p className="text-[11px] text-muted-foreground/50">
                      Batches are scoped per wallet — each wallet sees only its own batches
                    </p>
                  </div>
                ) : batches.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-14 gap-2">
                  ...
                  batches.map((b) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="font-mono text-[12px] text-foreground text-center">
                          {b.recipientCount}
                        </span>
                        ...
                        <span className="font-mono text-[11px] text-muted-foreground text-center">
                          {b.approvals} / {b.approvalThreshold}
                        </span>
                      </div>
                    );
                  })
