      ...
      <div className="min-h-full bg-background px-5 py-8 lg:px-8">
        <div className="max-w-2xl mx-auto space-y-5">
          {[0, 1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-24 rounded-2xl border border-hairline bg-surface animate-pulse"
            />
          ))}
        </div>
      </div>
        ...
        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-8">
          ...
          <motion.div variants={item}>
            ...
            <Card>
              ...
              <SettingRow label="Default asset" desc="Used when creating transfers">
                <select
                  value={form.defaultPaymentRail}
                  onChange={(e) => set("defaultPaymentRail", e.target.value)}
                  className="rounded-lg border border-hairline bg-background px-3 py-2 text-[12px] text-foreground outline-none focus:border-emerald/50 appearance-none"
                >
          ...
          <motion.div variants={item}>
            ...
            <Card>
              <SettingRow
                label="Privacy mode"
                desc="Enable confidential channel for all operations"
              >
              ...
              <SettingRow
                label="Shielded operator address"
                desc="Conceal workspace address from public chain queries"
              >
            ...
            <Card>
              <SettingRow label="Policy mode" desc="Who can decrypt transaction records">
                <select
                  ...
                  onChange={(e) => set("disclosurePolicy", e.target.value)}
                  className="rounded-lg border border-hairline bg-background px-3 py-2 text-[12px] text-foreground outline-none focus:border-emerald/50 appearance-none"
                >
          ...
          <motion.div variants={item}>
            ...
            <Card>
              <SettingRow label="Transfer settled" desc="Alert when a transfer reaches finality">
                <Toggle
                  enabled={form.notifyTransferSettled}
                  onChange={() => toggle("notifyTransferSettled")}
                />
              </SettingRow>
              <SettingRow
                label="Payroll approved"
                desc="Alert when batch reaches approval threshold"
              >
                <Toggle
                  enabled={form.notifyPayrollApproved}
                  onChange={() => toggle("notifyPayrollApproved")}
                />
              </SettingRow>
              <SettingRow label="Counterparty KYC" desc="Alert when KYC verification completes">
                <Toggle
                  enabled={form.notifyCounterpartyKyc}
                  onChange={() => toggle("notifyCounterpartyKyc")}
                />
              </SettingRow>
              <SettingRow label="System alerts" desc="Proof engine status and connectivity events">
                <Toggle
                  enabled={form.notifySystemAlerts}
                  onChange={() => toggle("notifySystemAlerts")}
                />
              </SettingRow>
            </Card>
          </motion.div>
          ...
          <motion.div variants={item} className="flex items-center gap-3">
            ...
            {saveMsg && (
              <span className="font-mono text-[11px] text-emerald animate-in fade-in">
                {saveMsg}
              </span>
            )}
          </motion.div>
