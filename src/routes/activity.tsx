...
const CAT_META: Record<string, { label: string; dot: string; badge: string }> = {
  ...
  treasury: {
    label: "Treasury",
    dot: "bg-violet",
    badge: "border-violet/20 bg-violet/8 text-violet",
  },
  counterparty: {
    label: "Counterparty",
    dot: "bg-foreground/40",
    badge: "border-hairline bg-surface-elevated text-foreground",
  },
  settings: {
    label: "Settings",
  ...
  system: {
    ...
    badge: "border-hairline bg-surface text-muted-foreground/70",
  },
};
            ...
            {FILTERS.map((f) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex items-center gap-1.5 rounded-full border px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider transition-all",
                    filter === f.id
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 gap-2">
                  ...
                  <p className="text-[11px] text-muted-foreground/50">
                    Activity is scoped per wallet — each wallet sees only its own events
                  </p>
                </div>
              ) : visible.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 gap-2">
                  <p className="text-[13px] text-muted-foreground">
                    {events.length === 0 ? "No activity yet" : "No events in this category"}
                  </p>
                  {events.length === 0 && (
                    <p className="text-[11px] text-muted-foreground/50">
                      Actions across transfers, payroll, and treasury will appear here
                    </p>
                  )}
                </div>
              ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="relative z-10 flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-hairline bg-surface">
                        <span className={`font-mono text-[12px] font-semibold ${statusIcon.col}`}>
                          {statusIcon.icon}
                        </span>
                      </div>
